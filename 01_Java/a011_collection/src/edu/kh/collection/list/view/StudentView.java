...
//			System.out.println(s.toString());
	...
	private void selectAddress() {
		...
		// 3학년 5반 2 번 홍길동 / 주소 : 서울시 중구
		for(Student s : list) {
			System.out.printf("%d학년 %2d반 %2d번 %s / 주소 : %S \n",
	...
	private void selectGrade() {
		...
		if(list.isEmpty()) {
			System.out.printf("[%d 학생이 존재하지 않습니다.]", input);
		} else {
			for(Student s : list) {
	...
	private void selectGender() {
		...
		while(true) { // 무한 반복
			...
			if(input == 'M' || input == 'F') { // 정상 입력
				...
				for(Student s : list) {
					System.out.printf("%d학년 %d반 %d번 %s (%s) \n",
...
//		if (list.isEmpty()) {
//		
//			System.out.print("[M 또는 F만 입력 해주세요.]");
//			 
//		}else 
//			
//		for(Student s : list) {
