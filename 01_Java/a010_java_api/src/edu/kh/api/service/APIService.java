...
public class APIService {
	...
	/** 학생 추가 서비스2 (equals(동등하다) / hashCode)
	 * @param grade
	 * @param classRoom
	 * @param numer
	 * @param name
	 * @return 추가 성공 시 true / 실패 시 false
	 */
	public boolean addStudent2(int grade, int classRoom, int numer, String name) {
		
		// 1. equals()를 이용한 비교
		
		// 1) 전달 받은 값으로 임시 학생 객체 생성
		Student temp = new Student(grade, classRoom, numer, name);
		...
		// s가 참조하는 학생과
		// temp가 참조하는 학생이 필드 값이 같을 경우 (동등한 경우)
	...
	public Student[] selectName(String input) {
		...
		for(Student s : studentList) {
			...
			if(s.getName().equals(n)) {
				result[index] = s; // result 배열에 일치하는 학생 저장(주소만 저장하여 얕은복사 함) 
				index++;
			}
		}
	}	
