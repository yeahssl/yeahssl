...
public class ArrayEx1 {
	...
	public void ex1() {
		...
		// 배열 선언
		int[] arr; 
		// - int 배열을 참조할 공간 할당하고 이를 arr이라고 부르겠다.
		...
		System.out.println("arr[3] : " + arr[3]); // 0~3, 1씩증가
	...
	public void ex2() {
		...
		// 2) Heap 영영게 새롭게 int 4칸짜리 배열을 할당
		//  -> index 번호 추가, 기본값(0)으로 초기화
		//   + 시작주소 부여 ex) 0x5123
		
		// 3) Heap영역에 생성된 배열의 시작 주소(0x5123)를
...
//		String[] menuArr = new String[10];
		...
		String[] menuArr = {"서브웨이", "KFC", "맘스터치","순대국", "뼈해장국", "닭갈비", "마라탕",
							"우육면", "파스타", "샐러드", "김밥일번지", "김밥천국", "쌀국수"};
	...
	public void ex5() {	
		...
		for(int i=0 ; i<=arr.length ; i++){ // i = 0,1,2,3,4,5 (index)
					// 5
	...
	public void ex6() {
		...
		int[] scoreArr = new int[size]; // 입력 받은 정수의 길이를 갖는 배열을 생성
