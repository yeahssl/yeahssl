...
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING (JOB_CODE)
WHERE HIRE_DATEIN( -- IN은 등호와 관련있는 연산자 이기 때문에 'NULL'이 포함되지 않음
...
-- 2) 어렵지만 신뢰도가 높은 결과
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME, HIRE_DATE, DEPT_CODE 
FROM EMPLOYEE MAIN
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING (JOB_CODE)
WHERE HIRE_DATE 
	=(
      ...
      WHERE ENT_YN = 'N' AND NVL(SUB.DEPT_CODE,'1') = NVL(MAIN.DEPT_CODE,'1')-- NULL이 아닌 경우(X)
	  
      --OR (ENT_YN = 'N'
	  	  --AND MAIN.DEPT_CODE IS NULL
	  	  --AND SUB.DEPT_CODE IS NULL) -- NULL인 경우(O)
      )
ORDER BY HIRE_DATE;	
...
-- 해결 2) 해결 1)의 조회 결과를 FROM절에 작성하여 상위 5행만 조회
SELECT ROWNUM, EMP_NAME, SALARY
FROM (SELECT EMP_NAME, SALARY 
	  FROM EMPLOYEE 
	  ORDER BY SALARY DESC) --> 메인쿼리에 포함된 VIEW 생성구문(인라인 뷰)
WHERE ROWNUM <= 5;
