...
-- WHERE EMP_ID SUBSTR(EMP_ID, -1,1) IN (1,3,5,7,9);
...
SELECT EMP_NAME, JOB_CODE, TO_CHAR(NVL((SALARY+(SALARY * BONUS))* 12,SALARY * 12),'L999,999,999') "연봉(원)" 
FROM EMPLOYEE;
...
--	- EXTRACT(DAY FROM HIRE_DATE)+1)) "입사한 달의 근무 일수"
...
SELECT EMP_NAME, DEPT_CODE,
	...
	FLOOR(MONTHS_BETWEEN(SYSDATE,TO_DATE((SUBSTR(EMP_NO,1,6)))) / 12) "만나이" 
--	SUBSTR(EMP_NO,1,2) || '년' || 
--	SUBSTR(EMP_NO,3,2) || '월' || 
--	SUBSTR(EMP_NO,5,2) || '일' "생년월일"
--	FLOOR((SYSDATE-TO_DATE((SUBSTR(EMP_NO,1,6))))/365) "만나이" 
--			-> 오차발생(4년에 한 번씩 있는 윤달을 고려)
FROM EMPLOYEE;
