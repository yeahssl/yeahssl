...
AND CATEGORY IN '인문사회';



-- 10. '음악학과' 학생들의 평점을 구하려고 한다. 음악학과 학생들의 "학번", "학생 이름", "전체 평점"을 출력하는
-- SQL 문장을 작성하시오.(단, 평점은 소수점 1자리까지만 반올림하여 표시한다.) 
SELECT TB_STUDENT.STUDENT_NO, TB_STUDENT.STUDENT_NAME, ROUND((
    SELECT AVG(TB_GRADE.POINT)
    FROM TB_GRADE
    WHERE TB_GRADE.STUDENT_NO = TB_STUDENT.STUDENT_NO
	), 1) AS 평점
FROM TB_STUDENT
JOIN TB_DEPARTMENT USING(DEPARTMENT_NO)
...
WHERE TB_STUDENT.COACH_PROFESSOR_NO = TB_PROFESSOR.PROFESSOR_NAME;
