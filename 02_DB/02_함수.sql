...
-- 문자열 앞,뒤 공백 제거(해당 용도로 가장 많이 사용됨)
...
--2023-03-07 12:23:09.000 ,2023-03-07 12:23:09.699 +0900
...
SELECT TO_CHAR(SYSDATE, 'YYYY"년" MM"월" DD"일"') TODAY FROM DUAL; 
...
-- CASE(여는괄호의 역할, ~한 경우)
-- 		WHEN(if문) 조건1 	THEN(중괄호의 역할) 결과1
-- 		WHEN(if문) 조건2 	THEN(중괄호의 역할) 결과2
-- 		WHEN(if문) 조건3 	THEN(중괄호의 역할) 결과3
--		ELSE 결과
-- END(닫는괄호의 역할)
...
SELECT MIN(SALARY), MAX(SALARY),
	   ...
	   MIN(HIRE_DATE)/*오래전에 입사한 사람*/, MAX(HIRE_DATE)/*최근에 입사한 사람*/
FROM EMPLOYEE;
