...
CREATE USER lys_sample IDENTIFIED BY "sample1234";
                                      --> 대소문자 구분
...
-- 3. 접속 권한 부여
-- GRANT 권한 [,권한,....] TO 계정명;
GRANT CREATE SESSION TO lys_sample;
...
------- > 다시 관리자 계정으로 전환해서 테이블 생석 관련 권한 부여
------------------------------ 관리자 계정으로 접속 해서 수행 ----------------------------------

-- 6. 테이블 생성 권한 부여
--    + 객체 생성 공간 할당
GRANT CREATE TABLE TO lys_sample; 

ALTER USER lys_sample DEFAULT TABLESPACE SYSTEM
...
GRANT SELECT ON EMPLOYEE TO lys_sample;
...
-- 4. 선동일 삭제
DELETE FROM  KH_LYS.EMPLOYEE 
WHERE EMP_ID = 200;
-- ORA-01031: 권한이 불충분합니다

----------------------------------kh 계정으로 접속해서 수행-------------------------------------
-- 5. sample 계정의 EMPLOYEE 테이블 조회 권한 회수
REVOKE SELECT ON EMPLOYEE FROM lys_sample;
 ...
 * */
------------------------------ 관리자 계정으로 접속 해서 수행 ----------------------------------
-- 1) RESOURCE : DB 사용을 위한 기본 객체 생성 권의 묶음
SELECT * FROM ROLE_SYS_PRIVS
WHERE ROLE = 'RESOURCE'; -- 8 행
...
GRANT CREATE SEQUENCE,
		...
		CREATE TYPE,
		CREATE TRIGGER,
		CREATE TABLE TO lys_sample;
	
--> ROLE을 이용해서 간단히 부여 / 회수
GRANT RESOURCE TO lys_sample;
REVOKE RESOURCE FROM lys_sample;

-- 사용자에게 부여된 시스템 권한 조회
-- SELECT * FROM DBA_SYS_PRIVS
-- WHERE GRANTEE = 'LYS_SAMPLE'; --> 계정 대문자로 쓰기
...
SELECT * FROM ROLE_SYS_PRIVS
WHERE ROLE = 'LYS_SAMPLE';

-- 앞으로 사용자 계정을 생성할 경우 권한 부여 구문
GRANT CONNECT, RESOURCE, CREATE VIEW TO lys_sample;
									  --> (계정명)
