...
CREATE TABLE MEMBER(
	...
	MEMBER_NAME VARCHAR2(15), -- 한글로 2글자 ~ 5글자 (한글 = 3BYTE)
	MEMBER_SSN  CHAR(14),	 -- 990123-1234567
...
COMMENT ON COLUMN MEMBER.ENROLL_DATE IS '회원 가입일'; -- ALT + X = 블록을 지정한 여러개의 컬럼실행
...
CREATE TABLE USER_USED_NN( -- NN : NOT NULL
...
-- ORA-01400: NULL을 ("KH_LYS"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
--	, UNIQUE(USER_ID)	-- 제약조건(컬럼명) (이름 미지정)
   ,CONSTRAINT USER_ID_UK UNIQUE(USER_ID)   -- (이름지정 O)
 );
...
-- ORA-00001: 무결성 제약 조건(KH_LYS.SYS_C007460)에 위배됩니다
-- ORA-00001: 무결성 제약 조건(KH_LYS.USER_ID_UK)에 위배됩니다
...
-- ORA-00001: 무결성 제약 조건(KH_LYS.USER_ID_NAME_UK)에 위배됩니다
...
CREATE TABLE USER_USED_PK( -- PK : PRIMARY KEY
...
-- 4. FOREIGN KEY(외부키 / 외래키) 제약조건 (난이도 ★★★★)
...
CREATE TABLE USER_USED_FK(
  ...
  -- 컬렘 레벨로 FK 제약조건 설정 (문제해결 시나리오로 내기 좋음 FOREIGN KEY가 있으면 REFERENCES로 바꿔야함)
...
--> NULL 사용 가능. (외부 테이블에 참조할 값이 없다 = 참조를 안한다)
...
--> 50이라는 값은 USER_GRADE 테이블 GRADE_CODE 컬럼에서 제공하는 값이 아니므로
 ...
 -- ORA-02291: 무결성 제약조건(KH_LYS.GRADE_CODE_FK1)이 위배되었습니다- 부모 키가 없습니다
...
-- ON DELETE SET NULL ★삭제 옵션★이 적용된 테이블 생성 (★★★ 반드시 기억 ★★★)
...
-- 5. CHECK 제약조건 : 컬럼에 기록되는 값에 조건 설정을 할 수 있음 (중요도 낮음)
-- CHECK (컬럼명 비교연산자 비교값)
...
-- ORA-02290: 체크 제약조건(KH_LYS.GENDER_CHECK)이 위배되었습니다
...
CREATE TABLE USER_TEST(
	...
	PNO VARCHAR(20) CONSTRAINT UK_PNO NOT NULL,
	GENDER VARCHAR2(3) CONSTRAINT CK_GENDER CHECK(GENDER IN ('남','여')),
	PHONE VARCHAR2(20), 
	ADDRESS VARCHAR2(100),
	STATUS VARCHAR2(3) DEFAULT 'N' CONSTRAINT NN_STATUS NOT NULL,
	CONSTRAINT CK_STATUS CHECK(STATUS IN ('N','Y')),
	CONSTRAINT UK_NPO UNIQUE (PNO)
);

COMMENT ON COLUMN USER_TEST.USER_NO IS '회원번호';
COMMENT ON COLUMN USER_TEST.USER_ID IS 'ID';
...
VALUES (1, 'USER1', '1234', '123456-7898765', '여', '010-1234-1234', '서울시 강남구 대치동','N');
INSERT INTO USER_TEST
VALUES (2, 'USER2', '2234', '123456-6898765', '남', '010-2234-1234', '서울시 강남구 역삼동','N');
INSERT INTO USER_TEST
VALUES (3, 'USER3', '3234', '123456-5898765', '여', '010-3234-1234', '서울시 강남구 삼성동','Y');
INSERT INTO USER_TEST
VALUES (4, 'USER4', '4234', '123456-4898765', '남', '010-4234-1234', '경기도 고양시 화정동','Y');
INSERT INTO USER_TEST
VALUES (5, 'USER5', '5234', '123456-3898765', '여', '010-5234-1234', '경기도 고양시 마두동','Y');
...
ALTER TABLE NATIONAL ADD CONSTRAINT FK_NATION_CODE
