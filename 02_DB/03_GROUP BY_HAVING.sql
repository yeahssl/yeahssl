...
-- 부서별 급여합, 급여평균, 인원수, 최고참 입사일, 막내 입사일 
...
SELECT DECODE(SUBSTR(EMP_NO,8, 1),'1', '남자', '2', '여자' ) "성별",
	   ...
	   COUNT(*) "인원 수" 
FROM EMPLOYEE 
GROUP BY DECODE(SUBSTR(EMP_NO,8, 1),'1', '남자', '2', '여자' ) --> 컬럼명, '함수' 사용 가능하다
			--> SELECT절 해석이 되지 않아 성별 별칭 사용 불가능
ORDER BY "인원 수";

-- WHERE + GROUP BY 
--> WHERE절이 GROUP BY절 보다 우선 순위가 빠르다!
...
-- EMPLOYEE 테이블에서 직급별 2000년도(2000년 이상)이후 입사자들의 급여 합 조회

SELECT JOB_CODE,SUM(SALARY)
FROM EMPLOYEE 
WHERE HIRE_DATE >= TO_DATE('2000/01/01')/*방법1*/
-- WHERE EXTRACT(YEAR FROM HIRE_DATE) >= 2000 /*방법2*/
GROUP BY JOB_CODE; 
...
--  * HAVING 절 : 그룹함수로 조회할 그룹에 대한 조거을 설정할 때 사용
...
-- (EX. 신고 1.게시물 신고 테이블, 2.댓글 신고 테이블, 3.사용자 신고 테이블 --> 신고 모아보기)
