...
CREATE TABLE "MEMBER" ( -- "" 안에 테이블명이 작성 되면, 대문자로만 인식할 수 있다.
	MEMBER_NO NUMBER CONSTRAINT MEMBER_PK PRIMARY KEY,
	MEMBER_ID VARCHAR2(30) NOT NULL,
	MEMBER_PW VARCHAR2(30) NOT NULL,
	MEMBER_NM VARCHAR2(30) NOT NULL,
	MEMBER_GENDER CHAR(1) CONSTRAINT GENDER_CHK 
  					  CHECK(MEMBER_GENDER IN ('M','F')),
  	ENROLL_DT DATE DEFAULT SYSDATE NOT NULL,
  	UNREGISTER_FL CHAR(1) DEFAULT 'N' CONSTRAINT UNREGISTER_CHECK 
  					  CHECK(UNREGISTER_FL IN ('Y','N'))
);
...
-- 게시글 목록 조회
-- 게시글 번호, 제목,  작성자(이름), 작성일, 조회수, 댓글 수(COMMENT_COUNT)
-- 최신 게시글이 위쪽으로 오도록 정렬
-- 댓글 수
SELECT BOARD_NO, BOARD_TITLE, MEMBER_NM, CREATE_DT, READ_COUNT,
...
AND MEMBER_ID = 'user1213113';

-- 회원 목록 조회(아이디, 이름, 성별(남/여) + 회원 번호 내림차순)
SELECT MEMBER_ID, MEMBER_NM, DECODE(MEMBER_GENDER, 'F', '여','남') 성별
FROM "MEMBER"
WHERE UNREGISTER_FL = 'N'
...
-- 댓글 삽입
INSERT INTO "COMMENT"
VALUES (COMMENT_NO, ?, DEFAULT, DELETE_FL, ?, ?)
;


-- 게시글에 해당하는 댓글
SELECT COMMENT_NO, MEMBER_NO, BOARD_NO 
FROM "COMMENT"
; 
